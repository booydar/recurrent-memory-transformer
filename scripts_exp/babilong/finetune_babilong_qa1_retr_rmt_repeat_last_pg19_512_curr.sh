...
BSS=(8 8 8 8 4 4 2 1)
LRS=(5e-05 5e-05 5e-05 3e-05 3e-05 3e-05 3e-05 1e-05)
...
MODEL_PATH=./runs/babilong/${TASK_DATASET}/${NOISE_DATASET}/$MODEL_NAME/lr${LR}_${SCHEDULER}_adamw_wd${WD}_seqlen${SAMPLE_SIZE}_${MAX_N_SEGMENTS}x${SEGMENT_SIZE}_mem${MEMORY_SIZE}_r${READ_MEM_SIZE}_w${WRITE_MEM_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_sp${SAMPLING_PROB}_retr_${RETR_MODE}_repeat_last_from_${SRC_N_SEGMENTS}x${SEGMENT_SIZE}/run_$N
INIT_MODEL_PATH=./runs/babilong/${TASK_DATASET}/${NOISE_DATASET}/$MODEL_NAME/lr${SRC_LR}_${SCHEDULER}_adamw_wd${WD}_seqlen${PREV_SAMPLE_SIZE}_${SRC_N_SEGMENTS}x${SEGMENT_SIZE}_mem${MEMORY_SIZE}_r${READ_MEM_SIZE}_w${WRITE_MEM_SIZE}_bs${TBS}_${SEGMENT_ORDERING}_bptt-${K2}_sp${SAMPLING_PROB}_retr_${RETR_MODE}_repeat_last_from_${SRC_SRC_N_SEGMENTS}x${SEGMENT_SIZE}/run_$N
...
accelerate launch --num_processes $NP --main_process_port 29161 --config_file $ACCEL_CONFIG run_finetuning_babilong_rmt.py \
